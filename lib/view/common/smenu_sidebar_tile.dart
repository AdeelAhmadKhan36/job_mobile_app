...
class _SidemenuTileState extends State<SidemenuTile> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        buildDivider(),
        buildListTile(3, Icons.manage_accounts_sharp, 'Device Managment'),
        buildDivider(),
        buildSectionHeader("History"),
        buildListTile(4, Icons.history, 'History'),
        buildDivider(),
  ...
  Widget buildSectionHeader(String text) {
    return Padding(
      padding: const EdgeInsets.only(left: 0, top: 50, bottom: 16, right: 170),
      child: Text(
        text.toUpperCase(),
...
class _HistoryState extends State<History> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        buildListTile(0, Icons.history, 'History'),
        Padding(
          padding: const EdgeInsets.only(left: 30),
          child: buildDivider(),
        ),
        buildListTile(1, Icons.notifications_active_rounded, 'Notifications'),
        buildDivider(),
      ],
    );
  }
  ...
  Widget buildListTile(int index, IconData icon, String title) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        duration: Duration(milliseconds: 300),
        decoration: BoxDecoration(
          color: selectedIndex == index ? Color(0xFF0000FF) : Colors.transparent,
          borderRadius: BorderRadius.all(Radius.circular(10)),
        ),
        child: ListTile(
          leading: SizedBox(
  ...
  Widget buildDivider() {
    return   Padding(
      ...
      child: Divider(height: 20, color: Colors.white24),
    );
  }
}
